...
analInterval = 1
timeInterval = 1.0
dataLocation = "steadyStateFlow/tests/firstDecentAttempt/"
lambdaMin = 0.01
lambdaMax = 1.5
concDiffMin = -0.2
concDiffMax = 0.2
    ...
    for concIndex in range(0, numConcs):
        ...
        while jobsStarted<numConcDiff:
            ...
            while (coresUsed<numCores) and (jobsStarted<numConcDiff):
                ...
                jobInput = "python steadyStateFlow.py "+str(botConc)+" "+str(topConc)+" "+str(currentRate)+" "+str(sysSize)+" "+str(analInterval)+" "+str(numStepsEquilib)+" "+str(numStepsAnal)+" "+str(timeInterval)+" "+str(transTime)+" "+dataLocation+str(rateIndex)+"/"+str(concIndex)+"/"+str(jobsStarted)+" >/dev/null"
