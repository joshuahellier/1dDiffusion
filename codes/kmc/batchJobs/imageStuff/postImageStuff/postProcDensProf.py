...
fileName = resultsPlace+"profileData.dat"
try:
    os.remove(fileName)
except OSError:
    pass
...
for i in directoryList:
    ...
    profInfo = []
    numTimeChunks = 4
    timeChunkSize = (finalTime-initialTime)/float(numTimeChunks)
    for siteIndex in range(2, sysSize-2):
        ...
        for timeChunkIndex in range (1, numTimeChunks-1):
            ...
            if True or siteIndex%2==0:
                typeFile.write(str(tempTotal/overallTotal)+" ")
            else:
                typeFile.write(str(1.0-tempTotal/overallTotal)+" ")
            if timeChunkIndex==2:
                profInfo.append(tempTotal/overallTotal)
        typeFile.write("\n")
    typeFile.close()
    fullProfInfo.append(profInfo)
    sites = []
    times = []
    steps = []
    types = []
...
with open (resultsPlace+"profileData.dat", 'w') as f:
    for profile in fullProfInfo:
        for site in profile:
            f.write(str(site)+" ")
        f.write("\n")
