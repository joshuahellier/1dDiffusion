...
# This code is designed to process the data from calculations created from the inputs generated by "lambdaFlucCreator.py". Based upon the particle creation and annihilation counts,
# it calculates the total current and its principle moments (mean, variance, skewness, kurtosis). It also calculates the mean and associated standard error for the particle density
# and the block size distribution. I'm not going to annotate this one in detail, as it's fairly pedestrian (lots of opening files, finding moments, and then putting those in new
# files). One thing to note is that it's designed with the fact that runs may have failed in mind; even for our best calculations on Eddie, around 5% of our calculations fail,
# so our postprocessing routines have to be able to cope with this.
