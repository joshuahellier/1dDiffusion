...
\subsection{Attempts to Investigate the Structure of the Flow}
Looking at the flow patterns shown in~\ref{fig:flowPatterns}, one can see that there is structure in there, which changes as we alter $\rho_M$ and $\lambda$; thus, one might reasonably wonder whether there is some correlation length which varies
as a function of $(\rho_M, \lambda)$.
\iffalse
